...
const actionSearch = async (discordClient, user, taskConfig) => {
  ...
  const searchOperations = searches.reduce((allSearches, { details, target }) => {
  ...
  // Perform searches for all search operations.
  await Promise.all(searchOperations.map(async (searchData, i) => {
    ...
    try {
      ...
      taskLogger.debug(searchInfo, `Search - wait: ${waitingTime} (${(i + 1)}/${searchOperations.length}), entries: ${search.entryCount}, new: ${newItems.length}, url: <${search.url}>`)
