...
const makeQueuedRequest = (url, cookieJar, extraHeaders, gzip, retries = REQUEST_TRIES, waitPeriod = WAIT_PERIOD) => {
  ...
  logger.silly(`calypso-request: created task ${requestID} (queue: ${queueNumber})`)
  ...
  return new Promise((resolve, reject) => {
    const task = setInterval(async () => {
      ...
      let tries = 0
      let latestError
      logger.silly(`calypso-request: starting task ${requestID}`)
      while (tries < retries) {
        // Warn if retrying the call.
        if (tries > 0) {
          logger.warn(`calypso-request: Request failed: ${latestError.code} - retry #${tries}: ${url}`)
        }
        tries += 1
        try {
      ...
      logger.silly(`calypso-request: giving up on task ${requestID}`)
      cleanFromQueue(requestID, queueNumber)
...
const cleanFromQueue = (requestID, queueNumber) => {
  ...
  for (let a = 0; a < queueOrder.length; ++a) {
    ...
    logger.silly(`calypso-request: deleting task ${requestID} (queue: ${a})`)
    delete queueOrder[a]
  }
...
const getQueueSpot = () => {
  ...
  logger.warn('calypso-request: Could not add a task to the queue.')
}
 ...
 */
export const requestURL = async (url, cookieJar, extraHeaders = {}, gzip = true) => {
  const req = await makeQueuedRequest(url, cookieJar, extraHeaders, gzip, REQUEST_TRIES, WAIT_PERIOD)
  return req
}
